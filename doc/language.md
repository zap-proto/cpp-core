...
Like Protocol Buffers and Thrift (but unlike JSON or MessagePack), Zap messages are
strongly-typed and not self-describing. You must define your message structure in a special
language, then invoke the Zap compiler (`zap compile`) to generate source code to
manipulate that message type in your desired language.

For example:

{% highlight zap %}
@0xdbb9ad1f14bf0b36;  # unique file ID, generated by `zap id`
...
* When evolving a protocol it is common to discover that some existing field really should have
  ...
  Zap's unions, it is actually possible to "retroactively unionize" such a field without
...
Zap's unconventional approach to unions provides these advantages without any real down
side:  where you would conventionally define a free-standing union type, in Zap you
...
group" in Zap, which was the case that got into the most trouble with Protobufs.
...
enums have numeric values, these numbers will be used, but in general Zap enums should not
...
it very easy to develop secure protocols with Zap -- you almost don't need to think about
access control at all. This feature is what makes Zap a "capability-based" RPC system -- a
...
Zap generics work very similarly to Java generics or C++ templates. Some notes:
...
* Only pointer types (structs, lists, blobs, and interfaces) can be used as generic parameters,
  ...
  which would excessively complicate the Zap implementation.
...
* The encoding of a generic type is exactly the same as the encoding of a type produced by
  ...
  <figure class="highlight"><pre><code class="language-zap" data-lang="zap"><span></span><span class="k">struct</span> <span class="n">PersonMap</span> {
...
You can define constants in Zap.  These don't affect what is sent on the wire, but they
will be included in the generated code, and can be [evaluated using the `zap`
tool](zap-tool.html#evaluating-constants).
...
{% highlight zap %}
struct Foo {
  baz @0 :import "bar.zap".Baz;
  # Use type "Baz" defined in bar.zap.
}
{% endhighlight %}
...
this case, the `zap` tool searches for the file in each of the search path directories specified
with `-I`, appending the path you specify to the path given to the `-I` flag. So, for example,
if you ran `zap` with `-Ifoo/bar`, and the import statement is `import "/baz/qux.zap"`, then
the compiler would open the file `foo/bar/baz/qux.zap`.

### Annotations

Sometimes you want to attach extra information to parts of your protocol that isn't part of the
Zap language.  This information might control details of a particular code generator, or
...
A Zap file must have a unique 64-bit ID, and each type and annotation defined therein may
also have an ID.  Use `zap id` to generate a new ID randomly.  ID specifications begin with `@`:
...
your file with the `-ozap` flag, which echos the schema back to the terminal annotated with
extra information, e.g. `zap compile -ozap myschema.zap`.  In general, you would only specify
...
but this would have some important disadvantages in the context of Zap:
...
* Any symbolic name can be changed, as long as the type ID / ordinal numbers stay the same.  Note
  ...
  you can use `zap compile -ozap myschema.zap` to find out what that number is, and then
...
* A non-generic type can be made [generic](#generic-types), and new generic parameters may be
  ...
  <figure class="highlight"><pre><code class="language-zap" data-lang="zap"><span></span><span class="k">struct</span> <span class="n">Map</span> {
    <span class="n">entries</span> <span class="nd">@0</span> <span class="nc">:List(Entry)</span>;
    <span class="k">struct</span> <span class="n">Entry</span> {
      <span class="n">key</span> <span class="nd">@0</span> <span class="nc">:Text</span>;
  ...
  <figure class="highlight"><pre><code class="language-zap" data-lang="zap"><span></span><span class="k">struct</span> <span class="n">Map</span>(<span class="n">Key</span>, <span class="n">Value</span>) {
    <span class="n">entries</span> <span class="nd">@0</span> <span class="nc">:List(Entry)</span>;
    <span class="k">struct</span> <span class="n">Entry</span> {
...
Also, these rules only apply to the Zap native encoding.  It is sometimes useful to
transcode Zap types to other formats, like JSON, which may have different rules (e.g.,

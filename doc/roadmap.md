...
This is a list of big ideas we'd like to implement in Zap. We don't know in what order
...
* **Bulk and Realtime**: Add features that make it easier to design Zap APIs for bulk
  data transfers (with flow control) and realtime communications (where it's better to drop
  messages than to deliver them late).
* **UDP transport**: Zap RPC could benefit from implementing a UDP transport, in order
  to achieve zero-round-trip three-party introductions and to implement "realtime" APIs (see
  "bulk and realtime", above).
* **Encrypted transport**: Zap RPC should support an encrypted transport which uses
...
* **Multi-threading:**  It should be made easy to assign different Zap RPC objects
...
* **JSON codec customization:**  Extend the JSON library to support customizing the JSON
  ...
  JSON than it is in Zap. The goal of these features would be to allow any reasonable
  pre-existing JSON schema to be representable as a Zap type definition, so that
  servers implementing JSON APIs can use Zap exclusively on the server side.
...
KJ is a framework library that is bundled with Zap, but is broadly applicable to C++
applications even if they don't use Zap serialization.
...
* **TLS bindings:** Write bindings for e.g. OpenSSL to make it easy to integrate with the KJ
  I/O framework, Zap RPC, and the KJ HTTP library.
...
* **Event loop integrations:** We should provide off-the-shelf integrations with popular event
  ...
  to use Zap RPC in applications that already use another event framework.
...
* **Schema compatibility checker:**  Add a `zap` command which, given two schemas, verifies
...
* **RPC debugger:**  Add a `zap` command which sends an RPC from the command line and prints
...
* **JSON-HTTP proxy:**  Develop a web server which can expose a Zap RPC backend as a
  JSON-over-HTTP protocol.
* **Database:**  A fast storage database based on Zap which implements the ORM interface

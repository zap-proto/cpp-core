---
layout: post
title: "Zap v0.3: Python, tools, new features"
author: kentonv
---

The first release of Zap came three months after the project was announced.  The second
...
now comfortably claim that Zap supports multiple languages.  [His Python
implementation](http://jparyani.github.io/pyzap/) wraps the C++ library and exposes
...
* Jason's implementation parses Zap schema files at runtime.  There is no need to run a
  ...
  import mechanism, so you can basically import a `.zap` schema file as if it were a `.py`
  module.  It's even convenient to load schema files and play with Zap messages from the
...
* It's _fast_.  Whereas the Python Protobuf implementation -- which we made the mistake of
  ...
  obscure problems), Jason's Zap implementation is faster than that, too.

Go [check it out](http://jparyani.github.io/pyzap/)!

By the way, there is also a budding [Erlang implementation](http://ezap.astekk.se/)
(by Andreas Stenius), and work
continues on [Rust](https://github.com/dwrensha/zap-rust) (David Renshaw) and
[Ruby](https://github.com/cstrahan/zap-ruby) (Charles Strahan) implementations.

### Tools: Zap on the Command Line

The `zap` command-line tool previously served mostly to generate code, via the `zap compile`
command.  It now additionally supports converting encoded Zap messages to a human-readable
text format via `zap decode`, and converting that format back to binary with `zap encode`.
These tools are, of course, critical for debugging.

You can also use the new `zap eval` command to do something interesting: given a schema file and
...
use Zap schemas as a config format: define your service configuration as a constant in
a schema file, importing bits specific to each client from other files that those clients submit
to you.  Use `zap eval` to "compile" the whole thing to binary for deployment.  (This has always
been a common use case for Protobuf text format, which doesn't even support variable substitution
or imports.)

Anyway, check out the [full documentation]({{ site.baseurl }}zap-tool.html) for
more.
...
* The schema API and `schema.zap` have been radically refactored, in particular to take advantage
...
* [Debian Unstable (sid)](http://www.debian.org/releases/sid/) now features
  [a Zap package](http://packages.debian.org/sid/zap), thanks to
...
* Popular OSX-based text editor [TextMate](http://macromates.com/) now
  [uses Zap internally](https://github.com/textmate/textmate/commit/5c02b4ff5cc0c7c319d3d4f127c8ee19b81f80b7),
  and the developer's feedback lead directly to several usability improvements included in this
  release.
* Many people using Zap _haven't bothered to tell us about it_!  Please, if you use it,
  [let us know](https://groups.google.com/group/zap) about your experience, both what you like

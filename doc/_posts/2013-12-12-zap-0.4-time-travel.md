---
layout: post
title: "Zap v0.4: Time Traveling RPC"
author: kentonv
---
...
Zap has a simple solution to this problem:  send call results _back in time_, so they
arrive at the client at the point in time when the call was originally made!

Curious how Zap bypasses the laws of physics?
...
promises alone (i.e. being asynchronous) constitutes "time travel".  Zap implements a
...
[check out the calculator example](https://github.com/kentonv/zap/blob/master/c++/samples)
for more._
...
[Promises/A+](http://promisesaplus.com/) and similar proposals.  Zap RPC introduces a
...
[Python bindings](http://jparyani.github.io/pyzap/) up to date, so you can already use RPC there
as well.  The Python interactive interpreter makes a great debugging tool for calling C++ servers.

### Up Next

Zap is far from done, but working on it in a bubble will not produce ideal results.
Starting after the holidays, I will be refocusing some of my time into an adjacent project which
will be a heavy user of Zap.  I hope this experience will help me discover first hand
the pain points in the current interface and keep development going in the right direction.

This does, however, mean that core Zap development will slow somewhat (unless contributors

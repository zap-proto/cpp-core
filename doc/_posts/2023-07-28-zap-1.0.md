...
* [A new class `zap::RevocableServer` was introduced](https://github.com/zap/zap/pull/1700) to assist in exporting RPC wrappers around objects whose lifetimes are not controlled by the wrapper. Previously, avoiding use-after-free bugs in such scenarios was tricky.

* Many, many smaller bug fixes and improvements. [See the PR history](https://github.com/zap/zap/pulls?q=is%3Apr+is%3Aclosed) for details.

## What's planned for 2.0?

The changes we have in mind for version 2.0 of Zap's C++ implementation are mostly NOT related to the protocol itself, but rather to the C++ API and especially to KJ, the C++ toolkit library that comes with Zap. These changes are motivated by our experience building a large codebase on top of KJ: namely, the Cloudflare Workers runtime, [`workerd`](https://github.com/cloudflare/workerd).

KJ is a C++ toolkit library, arguably comparable to things like Boost, Google's Abseil, or Facebook's Folly. I started building KJ at the same time as Zap in 2013, at a time when C++11 was very new and most libraries were not really designing around it yet. The intent was never to create a new standard library, but rather to address specific needs I had at the time. But over many years, I ended up building a lot of stuff. By the time I joined Cloudflare and started the Workers Runtime, KJ already featured a powerful async I/O framework, HTTP implementation, TLS bindings, and more.
...
Development of version 2.0 will take place on Zap's new `v2` branch. The `master` branch will become the 1.0 LTS branch, so that existing projects which track `master` are not disrupted by our changes.
...
* We will require a compiler with support for C++20, or maybe even C++23. Zap 1.0 only requires C++14.
...
* Zap's RPC API, KJ's HTTP APIs, and others are likely to be revised to make them more coroutine-friendly.
